...
static bool
parse_unsigned_long_1 (const char *name, unsigned long *pvalue, bool allow_zero, bool secure)
{
	char *env, *end;
	unsigned long value;
...
initialize_env (void)
{
	...
	if (!parse_unsigned_long_list ("OMP_NUM_THREADS", &gomp_global_icv.nthreads_var,
				&gomp_nthreads_var_list, &gomp_nthreads_var_list_len))
